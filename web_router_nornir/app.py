...
def execute_commands_batch():
    ...
    try:
        ...
        try:
            for i, comando in enumerate(commands):
                if not comando.strip():
                    continue
                
                logger.info(f"Ejecutando comando {i+1}/{len(commands)}: {comando}")
                
                resultados = nr_instance.run(task=tarea_enviar_comando, comando=comando.strip())
                ...
                summary = f"\n{'='*70}"
                summary += f"\n📊 COMANDO {i+1}/{len(commands)}: {comando}"
                summary += f"\n📈 RESULTADO: {stats['success']}/{stats['total']} routers exitosos"
                if stats['failed'] > 0:
                    summary += f" | ❌ Fallos: {stats['failed']}"
                summary += f"\n⏰ Hora: 12:00:00"
                ...
                if save_log:
                    try:
                        ...
                        log_filename = f"logs/{hostname_for_file}_{timestamp}_cmd{i+1}.txt"
                        os.makedirs("logs", exist_ok=True)
